...
source("utils.R")
library(arrow)
write_parquet(mpi_new_mun, "../datasets/mpi_new_mun.parquet")
#############################################
...
saveRDS(deaths, file = "../datasets/deaths.RDS")
...
saveRDS(mort_df, "../datasets/mort_df.RDS")
...
saveRDS(deaths_df_long, "../datasets/deaths_df_long.RDS")
...
saveRDS(geo_info_new_mun, file = "../datasets/geo_info_new_mun.RDS")
...
saveRDS(population, file = "../datasets/population.RDS")
saveRDS(population_new_mun, file = "../datasets/population_new_mun.RDS")
...
IMM_2020 <- read_excel("../datasets/IMM_2020.xls", skip = 5)
IMM_2015 <- read_excel("../datasets/IMM_DP2_2015.xlsx", sheet = "IMM_2015")
IMM_2010 <- read_excel("../datasets/IMM_DP2_2010.xlsx", sheet = "IMM_2010")
...
saveRDS(combined_index, "../datasets/mpi_mun.RDS")
grouped_municipality_50000 <- readRDS("../datasets/grouped_municipality_50000.RDS")
...
saveRDS(classified_data, file = "../datasets/mpi_classified.RDS")
...
saveRDS(index, "../datasets/mpi_new_mun.RDS")
...
saveRDS(index, file = "../datasets/marg_index.RDS")
...
saveRDS(index_new_mun, file = "../datasets/index_new_mun.RDS")
...
saveRDS(im.tmp, file = "../datasets/im.tmp.RDS")
saveRDS(index_marg, file = "../datasets/index_marg.RDS")
...
saveRDS(births, file = "../datasets/births.RDS")
grouped_municipality_50000 <- readRDS("../datasets/grouped_municipality_50000.RDS")
grouped_municipality_50000 <-
  grouped_municipality_50000 |>
  sf::st_drop_geometry()
...
births <- births |>
  ...
  summarise(births = sum(births), .groups = "drop")
saveRDS(births, file = "../datasets/births_new_mun.RDS")
...
for(file in file_list_fert) {
  ...
  saveRDS(df_processed, file = paste0("../datasets/month_fert_", year_reg, ".RDS"))
  print("File preprocessed")
}
...
yrs         <- 1990:2023
paths       <- sprintf("../datasets/month_fert_%d.RDS", yrs)
...
write_parquet(result, "../datasets/result.parquet")
month_births <- read_parquet("../datasets/monthly_births.parquet")
result[ ,
         delay := 12 * (as.numeric(year_reg)  - as.numeric(year_occ)) +
           (month_reg - month_occ)      # integer months
]

pop <- readRDS("../datasets/population_new_mun.RDS")

month_births <- result

saveRDS(month_births, file = "../datasets/month_births.RDS")
install.packages("arrow")      # only once
library(arrow)
write_parquet(month_births, "../datasets/monthly_births.parquet")
#-------------------------------------------------------------------------------
# Get a list of CSV files in the "fertility" folder
csv_files_fert <- list.files("../datasets/fertility", 
                             pattern = "fertility_mexico_[0-9]{4}\\.csv", 
...
saveRDS(births_long, file = "../datasets/births_long.RDS")


library(dplyr)
library(readr)

type_of_mun <- read_csv("../datasets/type_of_mun.csv") |>
  select(
...
rural_urban_area <- readRDS("../datasets/grouped_municipality_50000.RDS") |>
...
library(arrow)
write_parquet(rural_urban_area, "../datasets/rural_urban_area.parquet")
