...
from data_processing import BirthDataProcessor, BirthDataset
...
def main():
    ...
    config = {
        ...
        'nowcast_mc_samples': 200  # MC dropout samples for uncertainty quantification
    }
    ...
    # ==========================================
    # COLLECT ALL TRAINING DATA FOR FAST PLOTS
    # ==========================================
    print(f"\n📦 Collecting training artifacts for fast plotting...")
    
    # Extract training history from trainer
    training_history = trainer.history
    
    # Prepare complete training package
    complete_training_data = {
        # Raw test results (already computed)
        'test_predictions': results['predictions'].copy(),
        'test_targets': results['actuals'].copy(), 
        'test_variances': results['variances'].copy(),
        'test_sample_predictions': results['sample_predictions'].copy(),
        'test_alpha_values': np.array([param for param in trainer.model.alpha_embedding.weight.detach().cpu().numpy().flatten()]),
        
        # Training curves
        'train_losses': training_history['train_loss'].copy(),
        'val_losses': training_history['val_loss'].copy(),
        ...
        'test_metrics': {
            ...
            'rmse': results['rmse'],
            'mape': results['mape'],
            'mean_overdispersion': results['mean_overdispersion'],
            'theta_statistics': results['theta_statistics']
        }
    }
    
    print(f"✅ Collected {len(complete_training_data['test_predictions'])} test predictions")
    print(f"✅ Collected {len(complete_training_data['train_losses'])} training epochs")
    print(f"✅ Collected alpha parameters for {len(complete_training_data['test_alpha_values'])} municipalities")
    ...
    # Save model with all metadata AND complete training data
    final_model_path = './checkpoints/negative_binomial_model_final.pt'
    
    # ===============================================
    # COMPLETE TRAINING PACKAGE - FOR FAST PLOTTING
    # ===============================================
    complete_package = {
        # Core model data
        ...
        # Original results (backward compatibility)
        'training_results': training_results,
        'test_results': results,
        
        # ===== NEW: PRE-COMPUTED DATA FOR FAST PLOTS =====
        'fast_plot_data': complete_training_data,
        
        # Data processing state
        'data_processor_state': {
            # Save encoders with correct attribute names
            'le_municipality': processor.le_mun,
            'le_sex': processor.le_sex, 
            'le_age': processor.le_age,
            
            # Also save with original names for compatibility
            'municipality_encoder': processor.le_mun,
            'sex_encoder': processor.le_sex,
            'age_encoder': processor.le_age,
            
            # Additional processor attributes
            ...
            'min_births_threshold': getattr(processor, 'min_births_threshold', 5),
            'nowcast_cutoff_year': getattr(processor, 'nowcast_cutoff_year', 2016),
            
            # Save encoder classes for reconstruction
            'municipality_classes': processor.le_mun.classes_.tolist() if hasattr(processor.le_mun, 'classes_') else [],
            'sex_classes': processor.le_sex.classes_.tolist() if hasattr(processor.le_sex, 'classes_') else [],
            'age_classes': processor.le_age.classes_.tolist() if hasattr(processor.le_age, 'classes_') else [],
            
            # Save n_dimensions for model reconstruction
            'n_municipalities': len(processor.le_mun.classes_) if hasattr(processor.le_mun, 'classes_') else 520,
            'n_sex_categories': len(processor.le_sex.classes_) if hasattr(processor.le_sex, 'classes_') else 2,
            'n_age_groups': len(processor.le_age.classes_) if hasattr(processor.le_age, 'classes_') else 12
        },
        
        # Metadata
        'training_period': f"1990-{config['nowcast_cutoff_year']-1}",
        'model_type': 'negative_binomial',
        'distribution_formula': 'Var = μ + α_l·μ²',  # Corrected formula
        'saved_at': datetime.now().isoformat(),
        'fast_plot_ready': True  # Flag to indicate fast plot capability
    }
    
    torch.save(complete_package, final_model_path)
    
    print(f"✅ Final model saved to: {final_model_path}")
    print(f"   📊 Training period: 1990-{config['nowcast_cutoff_year']-1}")
    print(f"   🎯 Model type: Negative Binomial with municipality-specific α_l")
    print(f"   📈 Formula: Var = μ + α_l·μ²")
    ...
    print(f"   🚀 Fast plot data included: {len(complete_training_data['test_predictions'])} predictions")
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write(f"Municipality-specific parameters: 520 α_l values\n")
        f.write(f"CORRECTED Variance formula: Var = μ + α_l·μ²\n\n")
        ...
        f.write(f"\nFAST PLOTTING:\n")
        f.write(f"Pre-computed data available: YES\n")
        f.write(f"Training curves: {len(complete_training_data['train_losses'])} epochs\n")
        f.write(f"Test predictions: {len(complete_training_data['test_predictions'])} samples\n")
        f.write(f"\nMODEL FILES:\n")
    ...
    print(f"\n✅ TRAINING COMPLETED!")
    print(f"   ✓ Model trained on period: 1990-{config['nowcast_cutoff_year']-1}")
    print(f"   ✓ Model weights and configuration saved")
    print(f"   ✓ Data processor state saved")
    print(f"   ✓ Fast plotting data included")
    print(f"   ✓ Ready for separate nowcast predictions")
    
    print(f"\n📋 NEXT STEPS:")
    print(f"   1. Use 'nowcast_executor.py' to generate predictions")
    print(f"   2. Or create custom prediction scripts using saved model")
    print(f"   3. All encoder states and processor config are preserved")
