...
def main():
    ...
    def _build_triangle_for_nowcast(y_star, m_star, mun, sex, age, max_obs_delay):
        tri = np.zeros((P, D), dtype=np.float32)
        ref_id = y_star*12 + m_star
        g = df_all[(df_all['group_id'] == mun) &
                   (df_all['sex'] == sex) &
                   (df_all['age'] == age)]
        if g.empty:
            return tri
    ...
    def _observed_so_far(y_star, m_star, mun, sex, age, max_obs_delay):
        mask = (
    ...
    for i in range(0, len(target_rows), batch_size):
        ...
        for (y, m, mun, sex, age, max_obs) in chunk:
            tri = _build_triangle_for_nowcast(y, m, mun, sex, age, max_obs)
            tris.append(tri)
            mun_ids.append(processor.le_mun.transform([mun])[0])
            sex_ids.append(processor.le_sex.transform([sex])[0])
            age_ids.append(processor.le_age.transform([age])[0])
            po = processor.get_population_offset(mun, sex, age, y)
            pop_offs.append(np.log(po) if getattr(processor, 'use_population_offset', False) else 0.0)
            meta.append((y, m, mun, sex, age, max_obs))
        
        X = torch.tensor(np.stack(tris), dtype=torch.float32, device=device)
        MID = torch.tensor(mun_ids, dtype=torch.long, device=device)
        ...
        with torch.no_grad():
            samples = np.zeros((len(chunk), mc_samples), dtype=np.float32)
        ...
        for j, (y, m, mun, sex, age, max_obs) in enumerate(meta):
            obs = _observed_so_far(y, m, mun, sex, age, max_obs)
            rec = {
                'year_occ': y,
                'month_occ': m,
                'municipality': mun,
