...
source("../utils.R")  # Adjust path if needed

# --- 2. Load datasets ---
# Adjust file paths if needed
births <- read_parquet("births_new_mun.parquet")
deaths <- read_parquet("deaths_new_mun.parquet") |>
  filter(!age %in% 1:15)
population <- read_parquet("population_new_mun.parquet")|>
  filter(!age %in% c("00-04", "05-09", "10-14"))
marg_index <- read_parquet("marg_index.parquet")
# Note: 'rural_urban_area.parquet copia' may need renaming or path adjustment
rural_urban <- read_parquet("rural_urban_area.parquet copia")
