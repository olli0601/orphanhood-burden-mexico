# =============================================================================
# ch4_260_gam_archivio3_nowcast_2016_2023_barplots.R  ·  Chapter 4 — Delay-adjusted nowcasting
# Archived patched 2016-2023 barplots (line-CI variant).
# Reads input-data-processed/{municipality_models_proper_validation,births_grouped_mun,population_grouped_mun}.RDS -> output/ch4/*.png.
# =============================================================================
...
# Load trained models
municipality_models <- readRDS("input-data-processed/municipality_models_proper_validation.RDS")
...
if(nrow(nowcast_predictions) > 0) {
  ...
  nowcast_births_yearly <- nowcast_predictions %>%
    group_by(event_year, sex) %>%
    summarise(
      total_births_nowcast = sum(predicted, na.rm = TRUE),
      total_births_lower = sum(lower_95, na.rm = TRUE),
      total_births_upper = sum(upper_95, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    mutate(data_type = "nowcast")
  
  cat("Nowcast aggregated by year\n")
} else {
...
plot_data_nowcast <- nowcast_births_yearly %>%
  select(event_year, sex, total_births = total_births_nowcast, data_type)
...
# IMPORTANT: Nowcast should be TOTAL estimated births, stacked ABOVE registered births
stacked_data_2016_2023 <- registered_births_2016_2023 %>%
  ...
  mutate(
    ...
    # Nowcast additional = difference between total nowcast and registered (should be positive)
    births_nowcast_additional = pmax(0, total_births_nowcast - total_births_registered),
    # For verification
...
plot_data_combined <- bind_rows(
  ...
  mutate(
    ...
    # IMPORTANT: Order for stacking - Nowcast Additional FIRST (will appear on top), then Registered (bottom)
    type = factor(type, levels = c("Complete", "Nowcast Additional", "Registered")),
    births_thousands = births / 1000)
  
# === Overlay data for line + 95%% CI (thousands) ===
nowcast_ci_by_sex <- nowcast_births_yearly %>%
  mutate(
    births_thousands = total_births_nowcast / 1000,
    lower_thousands  = total_births_lower / 1000,
    upper_thousands  = total_births_upper / 1000
  )

nowcast_ci_total <- nowcast_births_yearly %>%
  group_by(event_year) %>%
  summarise(
    births_thousands = sum(total_births_nowcast, na.rm = TRUE) / 1000,
    lower_thousands  = sum(total_births_lower,  na.rm = TRUE) / 1000,
    upper_thousands  = sum(total_births_upper,  na.rm = TRUE) / 1000,
    .groups = "drop"
  )
...
p_female <- plot_data_combined %>%
  ...
  geom_ribbon(
    ...
    aes(x = event_year, ymin = lower_thousands, ymax = upper_thousands, fill = "Nowcast 95% CI"),
    inherit.aes = FALSE, alpha = 0.25
  ) +
  scale_fill_manual(values = c("Nowcast 95% CI" = "#81C784"), breaks = c("Nowcast 95% CI"), name = "Nowcast Model", guide = guide_legend(order = 1)) +
  geom_line(
    data = nowcast_ci_by_sex %>% dplyr::filter(sex == "female"),
    aes(x = event_year, y = births_thousands, color = "Nowcast Median"),
    inherit.aes = FALSE, linewidth = 1.3
  ) +
  scale_color_manual(values = c("Nowcast Median" = "#2E7D32"), breaks = c("Nowcast Median"), name = "Nowcast Model", guide = guide_legend(order = 1))+
...
saveRDS(nowcast_predictions, "input-data-processed/nowcast_predictions_2016_2023.RDS")
saveRDS(nowcast_births_yearly, "input-data-processed/nowcast_births_yearly.RDS")
saveRDS(stacked_data_2016_2023, "input-data-processed/stacked_births_data_2016_2023.RDS")
...
saveRDS(nowcast_summary, "input-data-processed/nowcast_summary_2016_2023.RDS")
