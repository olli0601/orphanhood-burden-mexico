# =============================================================================
# ch1_070_create_datasets_nowcasting.R  ·  Chapter 1 — Nowcasting inputs
# Build per-year birth datasets (CSV) for the monthly NowcastPNN model from the
# raw INEGI birth files (CSV 2017+ and DBF 1990-2016).
#
# Reads : input-data-raw/births/ (per-year csv + dbf)
# Writes: input-data-processed/fert_YYYY.RDS and per-year nowcasting CSVs
# Run after: ch1_010
# =============================================================================
...
csv_files_fert <- list.files(
  "input-data-raw/births/fertility",
  pattern = str_c("fertility_mexico_", year_regex, "\\.csv$"),
  full.names = TRUE
)

dbf_files_fert <- list.files(
  "input-data-raw/births/fertility_1990_2016",
