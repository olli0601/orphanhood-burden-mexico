...
ggsave("delay_cumulative_curve.png", width = 7, height = 5)
...
if(length(successful_results) > 0) {
  ...
  saveRDS(all_pred_cells, "nowcast_cell_predictions.RDS")
  cat("Salvate tutte le predizioni cella-per-cella in 'nowcast_cell_predictions.RDS'\n")
} else {
...
cat("\nSaving all results...\n")
saveRDS(all_results, "municipality_models_all.RDS")
if(exists("results_summary")) {
  saveRDS(results_summary, "municipality_models_summary.RDS")
}
