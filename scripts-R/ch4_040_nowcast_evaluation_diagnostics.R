...
if (!file.exists("input-data-processed/validation_predictions_corrected.RDS")) {
  stop("Error: validation_predictions_corrected.RDS not found.")
}

validation_data <- readRDS("input-data-processed/validation_predictions_corrected.RDS")
cat("Loaded validation predictions data:", nrow(validation_data), "rows\n")

# Also load test predictions if available
if (file.exists("input-data-processed/test_predictions_corrected.RDS")) {
  test_data <- readRDS("input-data-processed/test_predictions_corrected.RDS")
...
if (file.exists("input-data-processed/nowcast_predictions_2016_2023.RDS")) {
  nowcast_data <- readRDS("input-data-processed/nowcast_predictions_2016_2023.RDS")
...
if (file.exists("input-data-processed/stacked_births_data_2016_2023.RDS")) {
  stacked_births <- readRDS("input-data-processed/stacked_births_data_2016_2023.RDS")
...
saveRDS(evaluation_results, "input-data-processed/evaluation_top_bottom_municipalities.RDS")
